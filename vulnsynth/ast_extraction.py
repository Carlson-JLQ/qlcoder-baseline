...
import csv
try:
    from .config import AST_CACHE, VULNSYNTH_ROOT_DIR, get_chroma_client
except ImportError:
    from config import AST_CACHE, VULNSYNTH_ROOT_DIR, get_chroma_client 
...
async def run_codeql_query_with_bqrs(query_path: str, database_path: str, output_dir: str, logger) -> Tuple[str, str]:
    ...
    process = await asyncio.create_subprocess_exec(
        *cmd,
        ...
        cwd=VULNSYNTH_ROOT_DIR # Run from project root where qlpack.yml is
    )
...
select e, 
  ...
  l.getEndColumn() as endColumn""" 
            ...
            import shutil
            qlpack_source = f"{VULNSYNTH_ROOT_DIR}/qlpack.yml"
