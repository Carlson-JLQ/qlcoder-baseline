...
def _resolve_chroma_mcp_executable() -> str:
    """Resolve chroma-mcp to an absolute executable path when possible."""
    explicit = os.environ.get("CHROMA_MCP_PATH")
    if explicit and os.path.exists(explicit):
        return explicit

    candidates = []

    current_python_dir = os.path.dirname(os.sys.executable)
    candidates.append(os.path.join(current_python_dir, "chroma-mcp"))

    env_python = os.environ.get("PYTHON")
    if env_python:
        candidates.append(os.path.join(os.path.dirname(env_python), "chroma-mcp"))

    conda_exe = os.environ.get("CONDA_EXE")
    if conda_exe:
        conda_base = os.path.dirname(os.path.dirname(conda_exe))
        candidates.append(os.path.join(conda_base, "envs", "vulnsynth", "bin", "chroma-mcp"))

    home = os.path.expanduser("~")
    candidates.extend(
        [
            os.path.join(home, "anaconda3", "envs", "vulnsynth", "bin", "chroma-mcp"),
            os.path.join(home, "miniconda3", "envs", "vulnsynth", "bin", "chroma-mcp"),
        ]
    )
...
class ClaudeBackend(AgentBackend):
    ...
    async def execute_prompt(
        ...
        try:
            ...
            async def _write_stdin():
                try:
                    process.stdin.write(combined_prompt.encode())
            ...
            async def _read_stdout():
                ...
                while True:
                    ...
                    while b"\n" in buf:
                        ...
                        if phase_name.startswith("phase3_iter_") and not found_query_marker:
                            if "QUERY_FILE_PATH:" in decoded:
            ...
            t_in = asyncio.create_task(_write_stdin())
            ...
            while True:
                if process.returncode is not None:
                    break
                if found_query_marker and done_at and (time.time() - done_at) >= grace_after_done_sec:
                    break
                if time.time() >= deadline:
                    raise asyncio.TimeoutError()
                await asyncio.sleep(0.05)

            if process.returncode is None and found_query_marker:
                try:
                    process.terminate()
    ...
    def _generate_mcp_config(self, working_dir: str) -> str:
        ...
        chroma_mcp_exe = _resolve_chroma_mcp_executable()
        self.logger.info(f"Resolved chroma-mcp executable for desktop config: {chroma_mcp_exe}")

        if CHROMA_HOST:
            chroma_server = {
                "command": chroma_mcp_exe,
                "args": [
                    "--client-type", "http",
                    "--host", CHROMA_HOST,
    ...
    def _setup_project_scoped_chroma(self, workspace_dir: str):
        ...
        Resolves the chroma-mcp executable to an absolute path when possible.
        """
        chroma_mcp_exe = _resolve_chroma_mcp_executable()
        self.logger.info(f"Resolved chroma-mcp executable for project MCP: {chroma_mcp_exe}")
