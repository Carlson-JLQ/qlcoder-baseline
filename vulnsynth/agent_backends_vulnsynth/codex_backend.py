...
class CodexBackend(AgentBackend):
    ...
    async def execute_prompt(
        ...
        try:
            ...
            t_in = asyncio.create_task(write_stdin())
    ...
    def _write_codex_config(self, include_chroma: bool, include_codeql: bool):
        """Update ~/.codex/config.toml with the appropriate MCP servers."""
        ...
        # Preserve all unrelated config and only replace our managed MCP sections.
        cleaned_lines = []
        ...
        for line in existing_content.splitlines():
            ...
            if stripped.startswith("[") and stripped.endswith("]"):
                ...
                in_managed_section = (
                    section_name == "mcp_servers" or section_name.startswith("mcp_servers.")
                )
        ...
        if not existing_content and self.model_reasoning_effort:
            lines.append(f'model_reasoning_effort = "{self.model_reasoning_effort}"')
            lines.append("")
