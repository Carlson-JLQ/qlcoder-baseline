"""Coco-specific prompt functions for all ablation modes."""
...
_CHROMA_TOOL = "mcp__qlcoder_chroma__"
_CODEQL_TOOL = "mcp__qlcoder_codeql__"


def _phase1_payload_path(task) -> str:
    return f"{os.path.abspath(task.working_dir or '.')}/phase1_stdin.json"
...
**IMPORTANT**: Coco may use MCP/LSP tools to help edit or validate the query, but the automated system only trusts the final file persisted by `Write`.
...
def _retrieving_chroma_analysis(collection_name: str) -> str:
    return f"""### Retrieving Phase 1 Results
Use `{_CHROMA_TOOL}chroma_get_documents` with `collection_name="{collection_name}"` and:
...
Example:
```
{_CHROMA_TOOL}chroma_get_documents(
...
def _phase1_input_block(task) -> str:
    payload_path = _phase1_payload_path(task)
    return f"""
## Input

The preprocessed diff summary is saved as a JSON payload file for Coco.
You MUST use `Read` to open `{payload_path}` before starting the analysis.
Use that payload JSON as the primary input.
...
Use the raw diff only if the payload JSON summary is not sufficient.
"""
...
Use the payload JSON diff summary to identify:
- relevant files
- sources
- sinks
- sanitizers
...
def _coco_environment_note() -> str:
    return f"""
## Environment Notes For Coco

- This backend uses project-scoped MCP server names that are specific to VulnSynth:
  - Chroma: `{_CHROMA_TOOL}*`
  - CodeQL LSP: `{_CODEQL_TOOL}*`
- Actual MCP availability depends on the current ablation mode; if a tool family is unavailable in this run, do not attempt to call it.
- In full mode, stay inside repository tools + VulnSynth MCP tools; do not rely on web access, shell commands, or sub-agents.
- Prefer `Read`, `Write`, `Glob`, and `Grep` for repository inspection.
- Do not rely on stdin for task payloads; Phase 1 input is stored in `phase1_stdin.json`.
"""
...
{_coco_environment_note()}

## Objective
Analyze the vulnerability using the preprocessed diff summary stored in `{payload_path}` and return a structured Phase 1 JSON object.

{_phase1_input_block(task)}

### Step 1: Vulnerability Research (MANDATORY - Use Chroma MCP)
IMPORTANT: You MUST use the Chroma MCP server tools to research this vulnerability before producing the final JSON.

**Stage 1 - Get CVE Context:**
Query NIST first:
`{_CHROMA_TOOL}chroma_get_documents(collection_name="{task.nvd_cache}", where={{"cve_id": "{task.cve_id}"}})`

**Stage 2 - Context-Driven Searches:**
Based on NIST CWE and the payload JSON diff summary, search relevant collections with appropriate terms.

1. **CWE patterns**:
   `{_CHROMA_TOOL}chroma_query_documents(collection_name="cwe_data", query_texts=["CWE from NIST", "vulnerability type"], n_results=3)`

2. **CodeQL documentation**:
   `{_CHROMA_TOOL}chroma_query_documents(collection_name="codeql_language_guides", query_texts=["terms from diff summary"], n_results=3)`

3. **Local query examples**:
   `{_CHROMA_TOOL}chroma_query_documents(collection_name="codeql_local_queries", query_texts=["vulnerability category", "detection method"], n_results=3)`

4. **CodeQL reference**:
   `{_CHROMA_TOOL}chroma_query_documents(collection_name="codeql_ql_reference", query_texts=["taint tracking", "dataflow"], n_results=2)`

Do not call `{_CHROMA_TOOL}chroma_list_collections`.

### Step 2: Security Component Identification
Base the component extraction on:
- payload JSON diff summary from `{payload_path}`
...
1. Vulnerable AST:
   `{_CHROMA_TOOL}chroma_get_documents(collection_name="{task.ast_cache}", where={{"$and": [{{"cve_id": "{task.cve_id}"}}, {{"db_type": "vulnerable"}}]}})`
2. Fixed AST:
   `{_CHROMA_TOOL}chroma_get_documents(collection_name="{task.ast_cache}", where={{"$and": [{{"cve_id": "{task.cve_id}"}}, {{"db_type": "fixed"}}]}})`
...
1. Open it with `{_CODEQL_TOOL}codeql_update_file`
2. Run `{_CODEQL_TOOL}codeql_diagnostics`
3. If needed, use:
   - `{_CODEQL_TOOL}codeql_hover`
   - `{_CODEQL_TOOL}codeql_complete`
   - `{_CODEQL_TOOL}codeql_definition`
   - `{_CODEQL_TOOL}codeql_references`
4. Run `{_CODEQL_TOOL}codeql_format`
...
- `{_CODEQL_TOOL}codeql_diagnostics` reports no blocking errors
...
5. Validate with:
   - `{_CODEQL_TOOL}codeql_update_file`
   - `{_CODEQL_TOOL}codeql_diagnostics`
   - `{_CODEQL_TOOL}codeql_hover`
   - `{_CODEQL_TOOL}codeql_complete`
   - `{_CODEQL_TOOL}codeql_definition`
   - `{_CODEQL_TOOL}codeql_references`
   - `{_CODEQL_TOOL}codeql_format`
