...
def extract_last_agent_message(text: str) -> str:
    """Extract the last agent_message from JSONL output when available.

    This is intentionally phase-output specific and should not replace any
    backend's generic text extraction behavior. If the input is not JSONL with
    agent_message frames, return the original trimmed text.
    """
    ...
    return last_message if last_message else text.strip()


def _phase1_required_keys() -> set[str]:
    return {
...
def _parse_candidate_phase1_json(candidate: str) -> Optional[Dict[str, Any]]:
    try:
        parsed = json.loads(candidate)
    except Exception:
        return None
    if isinstance(parsed, dict) and _phase1_required_keys().issubset(set(parsed.keys())):
        return parsed
    return None


def _scan_json_objects_from_end(text: str) -> Optional[Dict[str, Any]]:
    """Find a valid Phase 1 JSON object by scanning opening braces from the end."""
...
def parse_phase1_json(text: str) -> Optional[Dict[str, Any]]:
    """Parse structured Phase 1 JSON from raw model output."""
    candidates = [text.strip()]

    last_message = extract_last_agent_message(text)
    if last_message and last_message not in candidates:
        candidates.insert(0, last_message)
    ...
    for candidate in candidates:
        parsed = _parse_candidate_phase1_json(candidate)
        if parsed is not None:
            return parsed

    # As a final fallback, scan from the end for the last complete legal JSON
    # object instead of using a greedy brace regex that can be confused by
    # snippets like `${tableName}` in progress messages.
    return _scan_json_objects_from_end(text)


def render_phase1_json_markdown(data: Dict[str, Any]) -> str:
    """Render structured Phase 1 JSON into readable Markdown."""
    lines: List[str] = []

    def add_text_section(title: str, value: str):
        if not value:
            return
    ...
    add_text_section("Vulnerability Research Summary", data.get("vulnerability_research_summary", ""))
    add_text_section("CVE Information", data.get("cve_information", ""))
    ...
    def add_component_section(title: str, items: List[Dict[str, Any]]):
        lines.append(f"## {title}")
        if not items:
            lines.append("None")
            lines.append("")
        ...
        for idx, item in enumerate(items, start=1):
            ...
            lines.append(f"   - Conceptual role: {item.get('conceptual_role', '')}")
            lines.append(f"   - Pattern category: {item.get('pattern_category', '')}")
            ast_elements = item.get("ast_elements", [])
            ast_text = ", ".join(ast_elements) if isinstance(ast_elements, list) else str(ast_elements)
            lines.append(f"   - AST elements: {ast_text}")
            lines.append(f"   - Detection strategy: {item.get('detection_strategy', '')}")
            lines.append("")

    add_component_section("Sources", data.get("sources", []))
    add_component_section("Sinks", data.get("sinks", []))
    add_component_section("Sanitizers", data.get("sanitizers", []))
    add_component_section("Additional Taint Steps", data.get("additional_taint_steps", []))
    add_text_section("Vulnerability Summary", data.get("vulnerability_summary", ""))
